...
{
	int user_len = strlen(dest_array);
	int prefix_len = strlen(prefix_array);
	int total_len = user_len + prefix_len +1;

	char *concat_array = malloc(total_len);
	strcpy(concat_array, prefix_array);
	strcat(concat_array,dest_array);
	return concat_array;
}
